...
| `swallowed-error` | medium | `catch {}` empty, or containing only a `console.log` that does not mention the caught error | a comment explaining the decision, a log that includes the error, anything else in the block | Retry, degrade or fail; if ignoring is right, log with context and say why |
...
| `sql-string-concat` | high | a string starting with SELECT/INSERT/UPDATE/DELETE/WHERE containing `${…}` or `' + var`, with a second SQL clause (FROM/INTO/SET/WHERE/VALUES/JOIN…) within two lines | no SQL clause nearby (an English "Update ${file}" is not SQL) | Parameterised queries |
