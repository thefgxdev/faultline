...
const TEXT_EXT = /\.(m?[jt]sx?|cjs|cts|mts|sql|py|php|rb|go|html?|vue|svelte|astro|env|ya?ml|json|toml|gitignore)$|(?:^|\/)Dockerfile[^/]*$/;
...
export function toMarkdown(result) {
  ...
  const rows = result.findings.map((f) => `| ${f.severity} | ${f.title} | \`${f.path}:${f.line}\` | \`${(f.evidence || '').replace(/\|/g, '\\|')}\` |`).join('\n');
