...
export async function collectFiles(root, ignore = []) {
  const out = [];
  out.rootNames = new Set(); // every entry name at the root, text or not (lockfiles, binaries)
  const walk = async (dir) => {
    ...
    for (const e of entries) {
      ...
      if (dir === root) out.rootNames.add(e.name);
...
export function matchGlob(glob, rel) {
  ...
  if (!re) {
    // tokenise once so replacements never touch each other's output
    let src = '';
    for (let i = 0; i < glob.length; i++) {
      const c = glob[i];
      if (c === '*') {
        if (glob[i + 1] === '*') { i++; if (glob[i + 1] === '/') { i++; src += '(?:.*/)?'; } else src += '.*'; }
        else src += '[^/]*';
      } else if (c === '?') src += '[^/]';
      else src += c.replace(/[.+^${}()|[\]\\]/, '\\$&');
    }
...
export async function scan(root, { ignore = [], minSeverity = 'info' } = {}) {
  ...
  const repo = { hasPackageJson, deps, hasLockfile: LOCKFILES.some((l) => files.rootNames.has(l)), hasEnvFile: [...rels].some((r) => /(?:^|\/)\.env(?:\.[^/]+)?$/.test(r) && !/example|sample|template/.test(r)), hasTenant: false };
  const texts = new Map();
